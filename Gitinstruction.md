...
В случае когда ветка неее была полностью слита (объеденена) с основной, Git покажет предупреждение и откажется удалять ветвь. Удалить ветвь которая ещеёё не полностью объединена, можно с помощью команды: **git branch -D ИмяВетки**.
